...
local function Updatec21ScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("c21_trackmat_"..id.."_left", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/2c1/tr",
	...
	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("c21_trackmat_"..id.."_right", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/2c1/tr",
...
--сдесь накладывается текстура на каккой либо объект (ent:SetSubMaterial( "выбор объекта", "!t26_trackmat_"..id.."выбор текстуры" ) )
	ent:SetSubMaterial( 2, "!c21_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 2, "!c21_trackmat_"..id.."_right" )
end

local function UpdateTracks()
	if c21s then
		for index, ent in pairs( c21s ) do
			if IsValid( ent ) then
				Updatec21ScrollTexture( ent )
			else
				c21s[index] = nil
			end
		end
...
net.Receive( "c21_register_tank", function( length )
	...
	if type == "c21" then
		table.insert(c21s, tank)	
	
	end
end)

net.Receive( "c21_update_tracks", function( length )
	local tank = net.ReadEntity()
	if not IsValid( tank ) then return end
...
local NumCycl = 0
hook.Add( "Think", "c21_manage_tanks", function()
