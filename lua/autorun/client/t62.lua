...
local function Updatet62ScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("t62_trackmat_"..id.."_left", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/t_62/tr",
	...
	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("t62_trackmat_"..id.."_right", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/t_62/tr",
...
--сдесь накладывается текстура на каккой либо объект (ent:SetSubMaterial( "выбор объекта", "!t26_trackmat_"..id.."выбор текстуры" ) )
	ent:SetSubMaterial( 1, "!t62_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 1, "!t62_trackmat_"..id.."_right" )
end

local function UpdateTracks()
	if t62s then
		for index, ent in pairs( t62s ) do
			if IsValid( ent ) then
				Updatet62ScrollTexture( ent )
			else
				t62s[index] = nil
			end
		end
...
net.Receive( "t62_register_tank", function( length )
	...
	if type == "t62" then
		table.insert(t62s, tank)	
	
	end
end)

net.Receive( "t62_update_tracks", function( length )
	local tank = net.ReadEntity()
	if not IsValid( tank ) then return end
...
local NumCycl = 0
hook.Add( "Think", "t62_manage_tanks", function()
