...
local function Updatet80ScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("t80_trackmat_"..id.."_left", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/simer/t80/track",
	...
	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("t80_trackmat_"..id.."_right", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/simer/t80/track",
...
--сдесь накладывается текстура на каккой либо объект (ent:SetSubMaterial( "выбор объекта", "!t26_trackmat_"..id.."выбор текстуры" ) )
	ent:SetSubMaterial( 1, "!t80_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 2, "!t80_trackmat_"..id.."_right" )
end

local function UpdateTracks()
	if t80s then
		for index, ent in pairs( t80s ) do
			if IsValid( ent ) then
				Updatet80ScrollTexture( ent )
			else
				t80s[index] = nil
			end
		end
...
net.Receive( "t80_register_tank", function( length )
	...
	if type == "t80" then
		table.insert(t80s, tank)	
	
	end
end)

net.Receive( "t80_update_tracks", function( length )
	local tank = net.ReadEntity()
	if not IsValid( tank ) then return end
...
local NumCycl = 0
hook.Add( "Think", "t80_manage_tanks", function()
