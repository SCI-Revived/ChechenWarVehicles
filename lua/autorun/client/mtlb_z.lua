...
local function Updatemt2ScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("mt2_trackmat_"..id.."_left", "VertexLitGeneric", 
	...
	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("mt2_trackmat_"..id.."_right", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/mb/tr",
...
--сдесь накладывается текстура на каккой либо объект (ent:SetSubMaterial( "выбор объекта", "!t26_trackmat_"..id.."выбор текстуры" ) )
	ent:SetSubMaterial( 1, "!mt2_trackmat_"..id.."_right" ) 
	ent:SetSubMaterial( 2, "!mt2_trackmat_"..id.."_left" )
end

local function UpdateTracks()
	if mt2s then
		for index, ent in pairs( mt2s ) do
			if IsValid( ent ) then
				Updatemt2ScrollTexture( ent )
			else
				mt2s[index] = nil
			end
		end
...
net.Receive( "mt2_register_tank", function( length )
	...
	if type == "mt2" then
		table.insert(mt2s, tank)	
	
	end
end)

net.Receive( "mt2_update_tracks", function( length )
	local tank = net.ReadEntity()
	if not IsValid( tank ) then return end
...
local NumCycl = 0
hook.Add( "Think", "mt2_manage_tanks", function()
