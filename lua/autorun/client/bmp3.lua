...
local function Updatebm3ScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("bm3_trackmat_"..id.."_left", "VertexLitGeneric", 
	...
	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("bm3_trackmat_"..id.."_right", "VertexLitGeneric", 
		{ ["$basetexture"] = "models/bm1/tr",
...
--сдесь накладывается текстура на каккой либо объект (ent:SetSubMaterial( "выбор объекта", "!t26_trackmat_"..id.."выбор текстуры" ) )
	ent:SetSubMaterial( 1, "!bm3_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 1, "!bm3_trackmat_"..id.."_right" )
end

local function UpdateTracks()
	if bm3s then
		for index, ent in pairs( bm3s ) do
			if IsValid( ent ) then
				Updatebm3ScrollTexture( ent )
			else
				bm3s[index] = nil
			end
		end
...
net.Receive( "bm3_register_tank", function( length )
	...
	if type == "bm3" then
		table.insert(bm3s, tank)	
	
	end
end)

net.Receive( "bm3_update_tracks", function( length )
	local tank = net.ReadEntity()
	if not IsValid( tank ) then return end
...
local NumCycl = 0
hook.Add( "Think", "bm3_manage_tanks", function()
