...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	vehicle:GetPhysicsObject():ApplyForceOffset( -shootDirection * 150000, shootOrigin ) 
		...
		projectile.CWV_PropDamageMultiplier = 0.54 -- 2700 prop damage, 27,000 damage per minute.
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(20,36,18), Vector(-10,60,120), true, "mg_muzzle" )
	
	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "muzzle", Type = 1 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-60,0,10), Vector(0,0,60), true, "muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end
	
	simfphys.RegisterCamera( vehicle.pSeat[2], Vector(0,0,0), Vector(0,0,0), true, "mg_muzzle" )
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	vehicle:SetPoseParameter("cannon_aim_pitch", math.Clamp(-TargetAng.p,-5 + pclamp,20) )
end
...
function simfphys.weapon:Attack( vehicle, ply, shootOrigin, Attachment, ID )
	...
	if self.M240Clip <= 0 then
		...
		self:SetNextFire( vehicle, CurTime() + 5 )
		return
	end
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( t80_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( t80_susdata[i].attachment ) ).Pos + Up * -4
		...
		vehicle:SetPoseParameter(t80_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	net.Start( "t80_update_tracks", true )
		net.WriteEntity( vehicle )
