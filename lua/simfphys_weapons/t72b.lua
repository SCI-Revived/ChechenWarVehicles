...
function simfphys.weapon:Initialize( vehicle )
	net.Start( "t72b_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "t72b" )
	net.Broadcast()
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(10,35,15), Vector(0,0,90), true, "mg_muzzle" )
...
if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end
	...
	if IsValid( pod ) then
		...
		if Toggle ~= vehicle.OldToggleC then
			...
			if Toggle then
				...
				if vehicle.tg_c_z then
					...
					simfphys.RegisterCamera( pod, Vector(10,35,15), Vector(0,0,90), true,"mg_muzzle")
					vehicle:SetBodygroup(2,0)
		...
		pod:SetLocalPos( Vector(-47,-130 + (vehicle.tg_c_z and 70 or 0),0+ (vehicle.tg_c_z and -45 or 0)))
		pod:SetLocalAngles( Angle(0,-90,0))
		ply:SetNoDraw(false)
		end
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( t72b_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( t72b_susdata[i].attachment ) ).Pos + Up * -4
		...
		vehicle:SetPoseParameter(t72b_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	net.Start( "t72b_update_tracks", true )
		net.WriteEntity( vehicle )
