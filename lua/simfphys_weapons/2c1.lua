...
local function hmg_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	vehicle:GetPhysicsObject():ApplyForceOffset( -shootDirection * 500000, shootOrigin ) 
	 ...
	 local projectile = {}
		...
		projectile.MuzzleVelocity = 7000
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( c21_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( c21_susdata[i].attachment ) ).Pos + Up * -4
		...
		vehicle:SetPoseParameter(c21_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	net.Start( "c21_update_tracks", true )
		net.WriteEntity( vehicle )
