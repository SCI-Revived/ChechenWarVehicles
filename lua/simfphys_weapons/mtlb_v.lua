...
local function mg_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	shootDirection = (shootDirection + Vector(
	 ...
	 local projectile = {}
		...
		projectile.CWV_PropDamageMultiplier = 0.4 -- 420 prop damage per shell, 5600 damage per minute.
...
function simfphys.weapon:Initialize( vehicle )
   net.Start( "mt1_register_tank" )
	...
	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Direction = Vector(0,0,1), Type = 1 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(-5,-6,-15), Vector(0,60,75), true, "muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "muzzle_machinegun", Type = 2 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(0,0,20), Vector(0,30,30), true )
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( mt1_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( mt1_susdata[i].attachment ) ).Pos
		...
		vehicle:SetPoseParameter(mt1_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	net.Start( "mt1_update_tracks", true )
		net.WriteEntity( vehicle )
